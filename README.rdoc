...
By default, pg_search_scope uses the built-in {PostgreSQL text search}[link:http://www.postgresql.org/docs/current/static/textsearch-intro.html]. If you pass the :features option to pg_search_scope, you can choose alternative search techniques.
...
* :tsearch - {Full text search}[link:http://www.postgresql.org/docs/current/static/textsearch-intro.html] (built-in with 8.3 and later, available as a contrib package for some earlier versions)
* :trigram - {Trigram search}[link:http://www.postgresql.org/docs/current/static/pgtrgm.html], which requires the trigram contrib package
* :dmetaphone - {Double Metaphone search}[link:http://www.postgresql.org/docs/9.0/static/fuzzystrmatch.html#AEN120188], which requires the fuzzystrmatch contrib package
...
Trigram support is currently available as part of the {pg_trgm contrib package}[http://www.postgresql.org/docs/current/static/pg_trgm.html] that must be installed before this feature can be used.
